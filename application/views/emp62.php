
<script src= '<?php echo base_url()."jquery/REWARDS.js"?>' type="text/javascript" ></script>

<br>
<br>
<br>

<div class="main-content">
                <div class="content-overlay"></div>
                <div class="content-wrapper">
<div class="container">
    <div class="content-header" style="    color: red;
    text-align: center;     margin-top: -17px;     margin-bottom: 35px;">            <i class="ft-copy"></i>
        المكافئات</div>
                    <div class="card card-custom gutter-b">
                        <div class="card-header flex-wrap border-0 pt-6 pb-0">
                            <div class="card-title">

                                <h3 class="card-label">
                                    <button type="button" class="btn btn-success bg-light-success btn-icon round mr-1 mb-1"><i class="fa fa-heart"></i></button>
                                    المكافئات
                                </h3>
                            </div>
                            <div class="card-toolbar">
                                <!--begin::Dropdown-->
                                <!--end::Dropdown-->
                                <!--begin::Button-->
                                <div class="">
                                    <!-- Button trigger modal-->
<!--                                    <button type="button" class="btn btn-success bg-light-success" data-toggle="modal"-->
<!--                                            data-target="#exampleModalSizeXl2"  id="AZBTN"> إضافة-->
<!--                                        <i class="fa fa-plus fa-1x">-->
<!--                                        </i>-->
<!--                                    </button>-->

                                    <div class="modal fade" id="update_emp_modalaz" tabindex="1" role="dialog"
                                         aria-labelledby="exampleModalSizeXl" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <button type="button" class="close" data-dismiss="modal"
                                                            aria-label="Close">
                                                        <i aria-hidden="true" class="ki ki-close"></i>
                                                    </button>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="row">


                                                        <div class="col-md-12">

                                                            <!--begin::Card-->
                                                            <div class="card card-custom gutter-b ">
                                                                <div class="card-header bg-light-light">
                                                                    <div class="card-title ">
                                                                        <h3 class="card-label labal-color-light"></h3>
                                                                        <h5 class="modal-title" id="exampleModalLabel"> الاجازات</h5>

                                                                    </div>
                                                                </div>

                                                                <div class="card-body ">
                                                                    <!--begin::Row-->
                                                                    <div class="row">
                                                                        <div class="col-md-4">
                                                                                <input type="hidden"  id="ID_REWARDS">
                                                                            <label for="" >    اسم الموظف
                                                                            </label>
                                                                                <select class="form-control  select2 " id="CARDID"
                                                                                        name="param" data-placeholder=" اسم الموظف   ">
                                                                                </select>



                                                                        </div>

                                                                        <div class="col-md-4">
                                                                            <label for=""
                                                                                   class="form__label control-label "
                                                                                   style="    left: 10rem  !important;">
                                                                                تاريخ الاجازة</label>

                                                                            <input type="text"
                                                                                       class=" form-control pickadate-selectors" data-date-format="dd-mm-yyyy"
                                                                                       placeholder=" " id="DATE_REWARDS">

                                                                        </div>

                                                                        <div class="col-md-4">
                                                                                <label for="" >     نوع الاجازة
                                                                                </label>
                                                                                <select class="form-control  select2 " id="TYPPE_REWARDS"
                                                                                        name="param" data-placeholder="  نوع الاجازة   ">

                                                                                    <option value="0">مرضية</option>
                                                                                    <option value="1">سنوية </option>
                                                                                    <option value="2">سنوية </option>
                                                                                </select>




                                                                        </div>

                                                                    </div>

                                                                        <div class="row">

                                                                            <div class="col-md-12">
                                                                                <label for="">
                                                                                    سبب الاجازة</label>
                                                                            <textarea class=" form-control" rows="2"

                                                                                      id="REASON_REWARDS"> </textarea>

                                                                            </div>

                                                                        </div>


                                                                    </div>







                                                                    <!--end::Row-->


                                                                <div class="card-footer bg-light-light text-center">
                                                                    <button
                                                                            class="btn gradient-mint "
                                                                            id="insert_btn" onclick="insert_REWARDS();"> إضافة <i
                                                                                class="fa fa-plus fa-1x">
                                                                        </i></button>
                                                                    <button
                                                                            class="btn gradient-purple-bliss" onclick="UPDATE_REWARDS();"
                                                                    > تعديل <i
                                                                                class="fa fa-edit fa-1x"> </i>
                                                                    </button>
                                                                    <button
                                                                            class="btn gradient-pomegranate"
                                                                            data-dismiss="modal"> اغلاق <i
                                                                                class="fa fa-window-close fa-1x"> </i>


                                                                </div>






                                                            </div>




                                                        </div>
                                                        <!--end::Card-->

                                                    </div>
                                                </div>
                                                <div class="modal-footer">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <!--end::Modal-->
                                </div>
                                <!--end::Button-->
                            </div>
                        </div>
                        <div class="card-body">
                            <!--begin: Datatable-->
                            <table class="table table-striped table-bordered dom-jQuery-events dataTable" id="get_REWARDS1">

                            </table>
                            <!--end: Datatable-->
                        </div>
                    </div>



        </div>
    </div>

</div>


